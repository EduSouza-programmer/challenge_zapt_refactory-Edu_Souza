...
describe("<UserFeedbackBase />", () => {
  ...
  describe("'Region' prop features", () => {
    ...
    it("should  render user region when is passed", () => {
      renderWithTheme(
        <UserFeedbackBase
          region={{ city: mock.region!.city, state: mock.region!.state }}
        />
      );

      const element = screen.getByText(
        `${mock.region?.city} • ${mock.region?.state}`
      );
    ...
    it("should render user region with the html span tag", () => {
      ...
      expect(
        screen.getByText(`${mock.region?.city} • ${mock.region?.state}`).tagName
      ).toEqual("SPAN");
    });
  });
